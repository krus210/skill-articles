...
object MarkdownParser {
    ...
    fun clear(string: String?): String? {
        ...
        while (isClearText == 0) {
            isClearText = 1
            val elements = mutableListOf<Element>()
            elements.addAll(findElements(newString))
            newString.clear()
            elements.forEach {
                newString.append(it.text)
                isClearText *= if (it.elements.isNotEmpty()) 0
                else 1
            }
        }
...
fun List<MarkdownElement>.clearContent() : String {
    return StringBuilder().apply {
        this@clearContent.forEach {
            when (it) {
                is MarkdownElement.Text -> it.elements.forEach { e1 -> append(e1.clearContent()) }
